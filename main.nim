...
proc rowEchelonForm(mat: Matrix): Matrix =
    ...
    while currentRowIndex < mat.rows:
        let 
            currentRowData = result.getRow(currentRowIndex)
        ...
        if currentRowDataPivotColumn == -1:
            if currentRowIndex != mat.rows - 1:
                ...
                if nextNonZeroRowIndex != -1:
                    result.swapRows(currentRowIndex, nextNonZeroRowIndex)
                    continue
                else:
                    break
            else:
        ...
        for subsequentRowIndex in (currentRowIndex + 1)..<mat.rows:
            let subsequentRowData = result.getRow(subsequentRowIndex)
            result.replaceRow(subsequentRowIndex, currentRowData.makeZeroValueAtColumn(subsequentRowData, currentRowDataPivotColumn))

        inc currentRowIndex
    
    result.makeSurePivotsHaveTheirAbsoluteValue
...
proc solutionExists(augmentedMatrix: Matrix): bool =
    ...
    let 
        rowEchelonForm = augmentedMatrix.rowEchelonForm
...
proc isInvertible(mat: Matrix): bool =
    ...
    result = mat.reducedRowEchelonForm == identityMat(mat.cols)
...
proc solveUsingRowReduction(augmentedMatrix: Matrix): string =
    ...
    let 
        ...
        neededVariables = allAlphabets[0..reducedRowEchelonForm.cols - 1]
    ...
    for rowIndex in countDown(augmentedMatrix.rows - 1, 0):
        ...
        let 
            rowData = reducedRowEchelonForm.getRow(rowIndex)
...
proc inverseFromRowOperations(mat: Matrix): Matrix =
    ...
    result = initMat(mat.cols, mat.rows)
    let 
    ...
    let augmentedMatrixContainingInverse = augmentedMatrix.reducedRowEchelonForm

    for rowIndex in 0..<mat.rows:
        let inverseRow = augmentedMatrixContainingInverse.getRow(rowIndex)[mat.cols .. ^1]
        result.replaceRow(rowIndex, inverseRow)
...
when isMainModule:
    ...
    try: 
        echo mat1
        # echo mat1.rowEchelonForm
        # echo mat1.determinant
        # echo mat1.inverse
        # echo mat1.determinant
        # echo mat1.inverseFromRowOperations
        # echo mat1.inverse
